...
// <copyright file="RedisCacheClient.cs" company="FIS" department="R&D">
...
namespace PatientPortal.Domain.Caching.Redis.StackExchange
{
    ...
    public abstract class RedisCacheClient:IDisposable
    {
        ...
        protected bool IsCachingServer = false;

        protected RedisCacheClient(ISerializer serializer)
        {
            IsCachingServer = RedisConfiguration.IsRedisCache;
        ...
        {
            if(!IsCachingServer) return new Dictionary<string, string>() { { "redis_version", "NO_DATA"} };
            return Sut.GetInfo();
        }
        public async Task<Dictionary<string, string>> ServerInfoAsync()
        {
            if (!IsCachingServer) return null;
        ...
        {
            if (!IsCachingServer) return false;
            try
            {
                if (key.Length < 1 || key == "*")
                {
        ...
        {
            if (data == null || !IsCachingServer) return false;
            try
            {
        ...
        {
            if (data == null || data.Count() < 1 || !IsCachingServer) return false;
            try
            {
        ...
        public bool AddList<T>(IEnumerable<T> data, params string[] paras)
        {
            ...
            try
            {
                ...
                foreach (var obj in data)
                {
                    ...
                    {
                        key = key + (key.Length < 1 ? "" : ":") + p + ":" + properties.Single(x => x.Name == p).GetValue(obj).ToString();
                    }
                    key = pattern + key;
                    objList.Add(Tuple.Create(key, obj));
        ...
        {
            if (!IsCachingServer) return default(T);
            var pattern = Helper.GetClassName<T>() + ":";
