angular.module('MyApp').controller("HomePageController", [
    "$scope", "$rootScope", "$location", "$route", "Params", "$timeout", "$q", "indexServices", 'MetaService',
    function ($scope, $rootScope, $location, $route, Params, $timeout, $q, indexServices, MetaService) {
        ...
        $scope.loadData = function () {
            ...
            $scope.combineResult = $q.all([
                ...
                if (resp[5].data != null) {
                    ...
                    //set meta
                    $rootScope.metaservice.setMetaPage($scope.contact.Name, $scope.contact.Name, $scope.contact.Name);
