...
namespace PatientPortal.DataAccess.CORE
{
    public class AppointmentLogImpl : IAppointmentLog
    {
        ...
        public async Task<bool> Confirm(AppointmentLogEdit data, char action)
        {
            try
            {
                var val = await _adapterPattern.SingleTransaction<AppointmentLogEdit, bool>(data, "usp_AppointmentLog_Confirm", action, DataConfiguration.instanceCore);
                return val;
            }
            catch (Exception ex)
            {
