...
namespace PatientPortal.API.Identity.Models
{
    public class PermissionManager
    {
        ...
        {
            var permission = await _permissionStore.FindByIdAsync(id);
            permission.PermissionRoles = _db.PermissionRoles.Where(p => p.PermissionId == id).ToList();
        ...
        public async Task<IdentityResult> UpdatePermissionRolesAsync(string roleId, List<string> permissionList)
        {
            ...
            if (prCount > 0)
            {
                try
                {
                    ...
                    {
                        //var temp = (from g in this.Permissions
                        //            where g.PermissionRoles.Any(u => u.RoleId == roleId));
