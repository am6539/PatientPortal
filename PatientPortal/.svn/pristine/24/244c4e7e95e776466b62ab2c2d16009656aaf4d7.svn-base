...
namespace PatientPortal.DataAccess.CORE
{
    public class UserImpl : IUser
    {
        ...
        public async Task<List<User>> GetUsersByGroupType(Dictionary<string, object> param)
        {
            try
            {
                var data = await _adapterPattern.ExecuteQuery<User>(param, "usp_User_ByGroupType", DataConfiguration.instanceCore);
                return data.ToList();
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> GetUserId(Dictionary<string, object> param)
        {
            try
            {
                return await _adapterPattern.SingleExecuteQuery<string>(param, "usp_User_FindByEmail", DataConfiguration.instanceCore);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
