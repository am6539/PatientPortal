...
GO
ALTER TABLE [dbo].[CMS] ALTER COLUMN [GoogleEngineScript] NVARCHAR (MAX) NULL;

ALTER TABLE [dbo].[CMS] ALTER COLUMN [GoogleMap] NVARCHAR (MAX) NULL;
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET
				...
				[Facebook] = @Facebook, [Google] = @Google, [Slogan] = @Slogan
			WHERE [Id] = 1
		END
		ELSE
		BEGIN
			INSERT [dbo].[CMS] ([Id], [Name], [Address], [Phone], [Fax], [Hotline], [GoogleMap], [GoogleEngineScript], [IsSendEmailWF],
								[OpeningHours], [Email], [Youtube], [Twitter], [Facebook], [Google], [Slogan]
								)
